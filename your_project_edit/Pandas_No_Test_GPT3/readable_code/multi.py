...
def _index_doc(kwargs: dict) -> dict:
    """
    Update and return index doc kwargs.

    Args:
    - kwargs: A dictionary of index documentation

    Returns:
    - dict: Updated index documentation kwargs
    """

    index_doc_kwargs = dict(kwargs)
    index_doc_kwargs.update(
        {"klass": "MultiIndex", "target_klass": "MultiIndex or list of tuples"}
    )

    return index_doc_kwargs
...
def _codes_to_ints(self, codes):
    ...
    codes_shifted = codes << self.offsets

    # Now sum and OR are in fact interchangeable. This is a simple
    # composition of the (disjunct) significant bits of each level (i.e.
    # each column in "codes") in a single positive integer:
    if codes_shifted.ndim == 1:
        # Single key
        return np.bitwise_or.reduce(codes_shifted)

    # Multiple keys
    return np.bitwise_or.reduce(codes_shifted, axis=1)
...
class MultiIndexPyIntEngine(
    ...
    """
    This class manages cases where the number of label combinations
    overflows 64-bit integers and uses an ObjectEngine containing Python integers.
    """
    ...
    def _codes_to_ints(self, codes):
        """
        Transform combinations of uint64 into Python integers in a strictly
        monotonic way, respecting the lexicographic order of integer combinations.
        ...
        # Avoid overflow by converting to Python integers:
        codes_shifted = codes.astype("object") << self.offsets

        # Combine significant bits of each level into a single positive integer
        # per row
        if codes_shifted.ndim == 1:
            # Single key
            return np.bitwise_or.reduce(codes_shifted)
...
def names_compat(meth: F) -> F:
    ...
    This makes it easier to share code with the base class.
    """

    @wraps(meth)
    def new_meth(self_or_cls, *args, **kwargs):
        """
        Check if only one of `name` or `names` is provided and handle accordingly.

        Args:
        self_or_cls: Instance of class or class itself
        *args: Variable length argument list
        **kwargs: Arbitrary keyword arguments

        Returns:
        Return the original method with updated kwargs
        """
        name_present = "name" in kwargs
        names_present = "names" in kwargs

        if name_present and names_present:
...
def _lexsort_depth(codes: list[np.ndarray], nlevels: int) -> int:
    ...
    Parameters:
    codes (list[np.ndarray]): List of arrays with codes
    nlevels (int): Maximum number of levels

    Returns:
    int: Depth with which codes are lexsorted
    """
    int64_codes = [_convert_to_int64(level_codes) for level_codes in codes]

    for k in range(nlevels, 0, -1):
        if _is_codes_lexsorted(int64_codes[:k]):
            return k

    return 0


def _convert_to_int64(array: np.ndarray) -> np.ndarray:
    """Convert numpy array to int64 type."""
    return array.astype(np.int64)


def _is_codes_lexsorted(codes: list[np.ndarray]) -> bool:
    """Check if codes are lexsorted."""
    return libalgos.is_lexsorted(codes)


def sparsify_labels(label_list, start: int = 0, sentinel: object = ""):
    """
    Rearranges the label_list with sparsification starting at the specified index.

    Args:
    label_list (list): List of labels to be sparsified.
    start (int): Start index for sparsification. Default is 0.
    sentinel (object): Sentinel value to indicate sparsification. Default is an empty string.

    Returns:
    list: Sparsified label list.
    """

    # Rearrange the label_list for sparsification
    pivoted = list(zip(*label_list))
    k = len(label_list)

    # Initialize the result list with the labels up to the start index
    ...
    for cur in pivoted[start + 1:]:
        ...
        for i, (p, t) in enumerate(zip(prev, cur)):
            # Check if we are at the last label
            if i == k - 1:
                sparse_cur.append(t)
...
def _get_na_rep(dtype: DtypeObj) -> str:
    """
    Returns the representation of NA values based on the data type.

    Args:
    dtype: DtypeObj. The data type object.

    Returns:
    str: The representation of NA values.
    """
    if isinstance(dtype, ExtensionDtype):
        # If dtype is an extension dtype, return its na_value
        return f"{dtype.na_value}"
    else:
        # If dtype is not an extension dtype, check the type and return the
        # corresponding NA representation
...
def maybe_droplevels(index: Index, key) -> Index:
    ...
    ----------
    index: Index
        The index that may have levels to be dropped.
    key : scalar or tuple
        If tuple, the caller is responsible for ensuring the key is not an entry in the first level of the MultiIndex.

    Returns
    -------
    Index
        The modified index after attempting to drop the specified levels.
    """
    original_index = index  # Saving the original index for fallback

    if isinstance(key, tuple):
        # If key is a tuple, try dropping levels mentioned in the key
        for _ in key:
            try:
                index = index._drop_level_numbers([0])
            except ValueError:
                # If dropping levels leads to an error, return the original
                # index
                return original_index
    else:
        # If key is not a tuple, try dropping the first level
        try:
            index = index._drop_level_numbers([0])
        except ValueError:
            # If dropping levels leads to an error, do nothing and proceed
            pass
...
def _coerce_indexer_frozen(
    ...
    array_like : array-like
        Input array-like object to be coerced.
    categories : array-like
        Array-like object containing categories.
    copy : bool
        Flag indicating whether to create a copy or not.

    Returns
    -------
    np.ndarray
        Non-writeable version of the coerced array.
    """
    # Coerce the array-like indexer to the appropriate dtype based on
    # categories
    array_like = _coerce_indexer_dtype(array_like, categories)

    # Make a copy if 'copy' parameter is True
    if copy:
        array_like = array_like.copy()

    # Make the array non-writeable before returning
...
def _require_listlike(level, arr, arrname: str):
    """
    Ensure that `level` is either None or list-like, and `arr` is a list of list-like objects.
    """
    # Check if `level` is not None and not list-like
    if level is not None and not is_list_like(level):
        # Check if `arr` is not list-like
        if not is_list_like(arr):
            raise TypeError(f"{arrname} must be list-like")
        # Check if `arr` is list-like, but its first element is also list-like
        if len(arr) > 0 and is_list_like(arr[0]):
            raise TypeError(f"{arrname} must be list-like")
        level = [level]
        arr = [arr]
    # Check if `level` is None or list-like
    elif level is None or is_list_like(level):
        # Check if `arr` is not list-like or its first element is not list-like
