# DEFAULT_PORTS dict for default port numbers
DEFAULT_PORTS = {"http": 80, "https": 443}

# Ensure that ', ' is used to preserve previous delimiter behavior.
# Generating default accepted encodings
# Using ',' as delimiter and splitting by ', ' to generate default
# accepted encodings
default_encodings = make_headers(accept_encoding=True)["accept-encoding"]
DEFAULT_ACCEPT_ENCODING = ", ".join(re.split(r",\s*", default_encodings))


def proxy_bypass_registry(host):
    """
    Return True if the host should be bypassed (proxy settings from registry on Windows).
    Checks proxy settings gathered from the registry.
    """
    try:
        import winreg
    except ImportError:
    ...
    # Query the proxy settings from the registry
    try:
    ...
    # Check if we need to bypass the proxy for the specified host
...
def proxy_bypass(host):
    """
    Return True if the host should be bypassed (proxy settings based on environment or registry).
    Checks proxy settings gathered from the environment or the registry.
    """
...
def requote_uri(uri):
    ...
    try:
        ...
        # unreserved, or '%')
        return quote(unquote_unreserved_chars(uri), safe=safe_with_percent)
    except InvalidURL:
        # We couldn't unquote the given URI, so let's try quoting it, but
        # there may be unquoted '%'s in the URI. We need to make sure they're
        # properly quoted so they do not cause issues elsewhere.
        return quote_uri(uri, safe=safe_without_percent)


def unquote_unreserved_chars(uri):
    """Unquote unreserved characters from the URI.

    :param uri: The URI to unquote.
    :type uri: str
    :return: The URI with unreserved characters unquoted.
    :rtype: str
    """
    return unquote(uri)


def quote_uri(uri, safe_chars):
    """Quote the URI using provided safe characters.

    :param uri: The URI to quote.
    :type uri: str
    :param safe_chars: Characters that should not be quoted.
    :type safe_chars: str
    :return: The quoted URI.
    :rtype: str
    """
    return quote(uri, safe=safe_chars)
