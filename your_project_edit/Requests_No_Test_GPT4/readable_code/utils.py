...
def super_len(o):
    """
    Calculate the total length of an object, taking into account different types
    of objects such as those with a length defined, files, or file-like objects.
    Returns the maximum size or length minus current position to support
    partially-read files.
    """

    # Initiate total_length to 0 as a default value
    total_length = 0
    # Current position within the file or object
    current_position = 0

    # Determine the total length based on the type or capabilities of the
    # object
    total_length = calculate_total_length(o)

    # Ensure the position is correctly adjusted for files/.tell()-supporting
    # objects
    current_position = adjust_current_position(o, total_length)

    # Calculate remaining length to read
    remaining_length = total_length - current_position
    return max(0, remaining_length)


def calculate_total_length(o):
    """
    Determine the total length of the object `o` using different methods,
    depending on the object's attributes.
    """

    # If the object directly supports length querying
    if hasattr(o, "__len__"):
        return len(o)

    # If object provides its length as a property
    elif hasattr(o, "len"):
        return o.len

    # If the object is a file or file-like, requiring more introspection
    elif hasattr(o, "fileno"):
        return calculate_file_length(o)

    # If none of the above, default to 0 length
    return 0


def calculate_file_length(o):
    """
    Determine the file length using file descriptor if possible, and also issue
    a warning if the file is not in binary mode but should be.
    """

    try:
        fileno = o.fileno()
    except (io.UnsupportedOperation, AttributeError):
        # File does not support fileno() or does but raised AttributeError,
        # skip length calculation, returning 0
        return 0
    else:
        total_length = os.fstat(fileno).st_size
        warn_if_not_binary(o)
        return total_length


def warn_if_not_binary(o):
    """
    Warns if the file-like object `o` has not been opened in binary mode when it
    should have been, due to os.fstat usage.
    """

    if "b" not in o.mode:
        warnings.warn(
            (
                "Requests has detected the content-length for this "
                "request using the binary size of the file; however, the "
                "file has been opened in text mode (i.e., without the 'b' "
                "flag in the mode). This may lead to an incorrect "
                "content-length. In Requests 3.0, support for files in text "
...
def adjust_current_position(o, total_length):
    """
    Adjust the current position, especially for file-like objects that support
    the .tell() method, in case it's been partially read.
    """

    # If the object supports the .tell() method, try adjusting its position
    if hasattr(o, "tell"):
        try:
            return o.tell()
        except OSError:
            # In the case of an OSError, return total_length if known, or 0
            # otherwise
            return total_length if total_length else 0
        else:
            # If total_length is unknown, try determining it by seeking
            if total_length is None:
                return calculate_length_from_seek(o)

    # If .tell() is not supported or not applicable, return 0
    return 0


def calculate_length_from_seek(o):
    """
    Determine the length of the object by seeking to the end and then back to
    the current position. Meant for file-like objects.
    """

    try:
        o.seek(0, 2)  # Seek to the end
        end_position = o.tell()
        o.seek(0)  # Seek back to the start
        return end_position
    except OSError:
        return 0
...
def should_bypass_proxies(url, no_proxy):
    """
    Determine if the given URL should bypass proxies based on the no_proxy setting.

    :param url: The target URL to check.
    :param no_proxy: A string of comma-separated domains that should bypass proxies.
    :rtype: bool
    """

    def get_no_proxy_setting(no_proxy_arg):
        """
        Retrieves the no_proxy setting from environment if not explicitly provided.

        :param no_proxy_arg: The initially provided no_proxy argument.
        :rtype: list
        """
        # Retrieve no_proxy from environment if not defined
        if no_proxy_arg is None:
            no_proxy_env = os.environ.get(
                'no_proxy') or os.environ.get('NO_PROXY')
            if no_proxy_env:
                return [
                    host for host in no_proxy_env.replace(
                        " ", "").split(",") if host]
        else:
            return [
                host for host in no_proxy_arg.replace(
                    " ", "").split(",") if host]
        return []

    def match_no_proxy(hostname, no_proxy_list):
        """
        Matches the hostname against the no_proxy list to determine bypass.

        :param hostname: Hostname of the URL.
        :param no_proxy_list: List of no_proxy entries.
        :rtype: bool
        """
        for host in no_proxy_list:
            # Match with and without port, and check CIDR for IP addresses
            if is_ipv4_address(hostname):
                if is_valid_cidr(host) and address_in_network(hostname, host):
                    return True
                elif hostname == host:
                    return True
            else:
    ...
    # Directly return True for URLs without hostname, e.g., file:///
    if parsed_url.hostname is None:
        return True

    no_proxy_list = get_no_proxy_setting(no_proxy)
    if no_proxy_list and match_no_proxy(parsed_url.hostname, no_proxy_list):
        # The URL matches the no_proxy setting, skip proxy
        return True

    # Fallback to system proxy settings if the environment-specific check
    # didn't bypass
    with set_environ("no_proxy", no_proxy):
        try:
            return proxy_bypass(parsed_url.hostname)
